...
export class CatalogService {
  ...
  /**
   * Adds a new action to the catalog
   * @param action - The action to add
   * @returns The updated catalog
   */
  public async addAction(action: CatalogAction): Promise<ActionCatalog> {
    const catalog = this.catalog || await this.loadCatalog();
    
    // Check if action with the same ID already exists
    const existingIndex = catalog.actions.findIndex(a => a.id === action.id);
    
    if (existingIndex >= 0) {
      // Update existing action
      catalog.actions[existingIndex] = action;
    } else {
      // Add new action
  ...
  /**
   * Adds a new action definition to the catalog
   * @param actionDefinition - The action definition to add
   * @returns The ID of the added action
   */
  public async addActionDefinition(actionDefinition: ActionDefinition): Promise<string> {
    // Convert ActionDefinition to CatalogAction
    const catalogAction: CatalogAction = {
      id: actionDefinition.id,
      name: actionDefinition.name,
      description: actionDefinition.description,
      category: 'Generated Wrappers',
      repository: actionDefinition.repository,
      workflowPath: actionDefinition.wrapperPath,
      inputs: Object.entries(actionDefinition.inputs).map(([name, input]) => ({
        name,
      ...
      documentation: `# ${actionDefinition.name}\n\n${actionDefinition.description}\n\nThis is a generated wrapper for the GitHub Action: ${actionDefinition.originalActionUrl}`,
      tags: ['generated', 'wrapper', 'mcp'],
      lastUpdated: new Date().toISOString(),
      version: actionDefinition.generationMetadata.actionVersion
    };
    
    // Add the action to the catalog
    await this.addAction(catalogAction);
    
    // Ensure the "Generated Wrappers" category exists
    await this.ensureCategoryExists({
      id: 'generated-wrappers',
      name: 'Generated Wrappers',
      description: 'Actions generated by the MCP wrapper system'
    });
  ...
  /**
   * Ensures that a category exists in the catalog
   * @param category - The category to ensure exists
   */
  private async ensureCategoryExists(category: ActionCategory): Promise<void> {
    const catalog = this.catalog || await this.loadCatalog();
    
    // Check if category already exists
    const existingCategory = catalog.categories.find(c => c.id === category.id);
...
// Create a singleton instance with a default GitHub client
// In a real implementation, this would use the shared GitHub client
const catalogService = new CatalogService(new GitHubApiClient(''));
